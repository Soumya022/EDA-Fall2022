...
print(Plot_text)
#function expression is needed to have mu and R^2
...
ggplot(PeterPaul.chem.nutrients, aes(x = daynum, y = temperature_C)) + geom_point(aes(color = year4)) + facet_wrap(vars(lakename),nrow=2) + ylab("Surface Temp")

summary(PeterPaul.chem.nutrients$lakename)
...
ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C)) + geom_point(aes(color = depth), size = 0.5) + ylab("Temp in C") + scale_color_viridis(option = "magma", direction = -1) + scale_x_date(date_breaks = "5 years", date_labels = "%Y")
...
ggplot(EPAair, aes(x = PM2.5, y = Ozone)) + geom_point(aes(color = meanLat), alpha = 0.5) + geom_smooth(method =lm, color = "red") + ylab(expression("0"[3]*))
